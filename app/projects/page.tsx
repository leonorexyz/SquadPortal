...
const projects: Project[] = [
  { id: "website-redesign", name: "Website Redesign", description: "A clearer, calmer home for the product and the people using it.", client: "Nexa Labs", status: "Development", statusClass: "development", visibility: "Internal", owner: "Nadia Putri", ownerId: "nadia-putri", initials: "NP", tasks: 18, completedTasks: 13, due: "2026-09-12", color: "purple" },
  { id: "mobile-app", name: "Mobile App v2", description: "Reworking the core mobile workflow around faster everyday decisions.", client: "IFabula", status: "SIT", statusClass: "sit", visibility: "Internal", owner: "Raka Aditya", ownerId: "raka-aditya", initials: "RA", tasks: 24, completedTasks: 21, due: "2026-09-05", color: "green" },
  { id: "q3-campaign", name: "Q3 Campaign", description: "A focused campaign to help more teams discover the new workspace.", client: null, status: "UAT", statusClass: "uat", visibility: "Public", owner: "Dimas Pratama", ownerId: "dimas-pratama", initials: "DP", tasks: 13, completedTasks: 6, due: "2026-09-27", color: "orange" },
  { id: "team-ops", name: "Team Operations", description: "Small systems that make planning, handoffs, and collaboration easier.", client: "Leonore Kingdom", status: "Implementation", statusClass: "implementation", visibility: "Internal", owner: "Sarah Anderson", ownerId: "demo-user", initials: "SA", tasks: 16, completedTasks: 10, due: "2026-09-19", color: "blue" },
  { id: "research-library", name: "Research Library", description: "Organizing the insights that help us make better product decisions.", client: "Northstar", status: "Go-live", statusClass: "go-live", visibility: "Public", owner: "Sinta Maheshwari", ownerId: "sinta-maheswari", initials: "SM", tasks: 11, completedTasks: 11, due: null, color: "purple" },
  { id: "onboarding-refresh", name: "Onboarding refresh", description: "A welcoming first week for every new member joining the squad.", client: "Leonore Kingdom", status: "Preparation", statusClass: "preparation", visibility: "Internal", owner: "Sarah Anderson", ownerId: "demo-user", initials: "SA", tasks: 9, completedTasks: 3, due: "2026-10-02", color: "pink" },
];
...
function mapApiProject(project: ApiProject, index: number, currentUser: { id: string; name: string }, ownerMap: Map<string, ProjectOwner> = new Map()): Project {
  const staticProject = staticProjectById.get(project.id);
  const owner = ownerMap.get(project.ownerId)?.name ?? (project.ownerId === currentUser.id ? currentUser.name : project.ownerId);
  return {
    id: project.id,
    name: project.name,
...
type ProjectFormState = Pick<Project, "name" | "description" | "client" | "status" | "visibility" | "ownerId" | "due">;

function emptyProjectForm(ownerId = "demo-user"): ProjectFormState {
  return {
  name: "",
  description: "",
  client: "",
...
export default function ProjectsPage() {
  ...
  const [projectForm, setProjectForm] = useState<ProjectFormState>(() => emptyProjectForm(requestUserId));
  ...
  const ownerMap = useMemo(() => new Map(projectOwners.map((owner) => [owner.id, owner])), [projectOwners]);
  ...
  useEffect(() => {
    ...
    setIsLoading(true);
    setRequestError("");
    void Promise.all([fetchProjects(requestUserId, controller.signal), fetchProjectOwners(requestUserId, controller.signal)])
      .then(([data, owners]) => {
        setProjectOwners(owners);
        const nextOwnerMap = new Map(owners.map((owner) => [owner.id, owner]));
        setProjectList(data.map((project, index) => mapApiProject(project, index, currentUser, nextOwnerMap)));
      })
      .catch((error) => {
  ...
  function openCreateProject() {
    ...
    setProjectForm(emptyProjectForm(requestUserId));
    setRequestError("");
  ...
  function openEditProject(project: Project) {
    ...
    setProjectForm({ name: project.name, description: project.description, client: project.client, status: project.status, visibility: project.visibility, ownerId: project.ownerId, due: project.due });
    setIsProjectFormOpen(true);
  }
  ...
  async function handleProjectSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setProjectList(refreshedProjects.map((project, index) => mapApiProject(project, index, currentUser, ownerMap)));
  ...
  return <div className="dashboard-shell">
     ...
     <main className="main-content projects-page"><div className="mobile-header"><ProjectBrand /><PortalUserAvatar className="avatar-header" /></div><header className="main-header"><div><p className="breadcrumb"><strong>Workspace</strong> <span>/</span> Projects</p><h1 className="page-title">Projects</h1><p className="page-subtitle">A shared view of everything the team is moving forward.</p></div><button className="primary-button project-create-button" type="button" onClick={openCreateProject}><Plus size={15} strokeWidth={2} /> New project</button></header>
      ...
      {isProjectFormOpen ? <ProjectFormModal form={projectForm} owners={projectOwners} ownersLoading={isLoading} editing={editingProjectId !== null} submitting={isSubmitting} onChange={setProjectForm} onClose={closeProjectForm} onSubmit={handleProjectSubmit} /> : null}
