...
export default function KnowledgePage() {
  ...
  return <div className="dashboard-shell"><aside className="sidebar" aria-label="Main navigation"><KnowledgeBrand /> <PortalNavigation /><div className="sidebar-bottom"><PortalSettingsLink /><PortalUserProfile roleLabel="Product lead" /></div></aside><main className="main-content knowledge-page"><div className="mobile-header"><KnowledgeBrand /><PortalUserAvatar className="avatar-header" /></div><header className="main-header"><div><p className="breadcrumb"><strong>Workspace</strong> <span>/</span> Knowledge base</p><h1 className="page-title">Knowledge base</h1><p className="page-subtitle">The team context, decisions, and guides you can come back to.</p></div><button className="primary-button" type="button" onClick={openDocumentForm}>+ New document</button></header>{isLoading ? <KnowledgeSummarySkeleton /> : <section className="knowledge-summary" aria-label="Knowledge base summary"><div><span>All documents</span><strong>{documentList.length}</strong><small>Loaded from the workspace</small></div><div><span>Updated this week</span><strong>{documentList.length}</strong><small>Keep the context fresh</small></div><div><span>Most represented</span><strong>{documentList[0]?.category ?? "—"}</strong><small>From current results</small></div></section>}<div className="knowledge-toolbar"><div className="search-wrap knowledge-search"><Search size={16} strokeWidth={1.8} aria-hidden="true" /><input className="search-input" type="search" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search the knowledge base..." aria-label="Search knowledge base" /></div><select className="select-control" value={category} onChange={(event) => setCategory(event.target.value)} aria-label="Filter documents by category"><option>All categories</option><option>Design</option><option>Engineering</option><option>Research</option><option>Team</option><option>Product</option><option>Operations</option></select></div>{requestError && <p className="field-error" role="alert">{requestError}</p>}{isLoading ? <KnowledgeCardGridSkeleton /> : <section className="knowledge-card-grid" aria-label="Knowledge documents">{documentList.map((document) => <KnowledgeCard key={document.id} document={document} accessLevel={documentAccess[document.id] ?? accessLevelFor(document)} onAccess={() => openAccessPanel(document)} onEdit={() => openEditDocument(document)} onDelete={() => setDocumentToDelete(document)} />)}{documentList.length === 0 && <p className="empty-search">No documents match your search.</p>}</section>}{isDocumentFormOpen && <DocumentFormModal form={documentForm} editing={editingDocument !== null} submitting={isSubmitting} onChange={setDocumentForm} onClose={closeDocumentForm} onSubmit={handleDocumentSubmit} />}{selectedAccessDocument && <AccessPanel document={selectedAccessDocument} level={accessLevel} members={accessMembers} onLevelChange={setAccessLevel} onToggleMember={toggleAccessMember} onClose={() => setSelectedAccessDocument(null)} onSave={saveDocumentAccess} />}{documentToDelete && <DeleteDocumentModal document={documentToDelete} busy={isSubmitting} onCancel={() => setDocumentToDelete(null)} onConfirm={confirmDeleteDocument} />}</main></div>;
}

function KnowledgeSummarySkeleton() {
  return <section className="knowledge-summary loading-summary-skeleton" aria-label="Loading knowledge summary" aria-busy="true">{[1, 2, 3].map((item) => <div key={item}><span className="loading-skeleton skeleton-summary-label" /><span className="loading-skeleton skeleton-summary-value" /><span className="loading-skeleton skeleton-summary-detail" /></div>)}</section>;
}

function KnowledgeCardGridSkeleton() {
  return <section className="knowledge-card-grid loading-card-grid" aria-label="Loading knowledge documents" aria-busy="true">{[1, 2, 3, 4, 5, 6].map((item) => <article className="knowledge-card loading-card-skeleton" key={item}><span className="loading-skeleton skeleton-card-accent" /><span className="loading-skeleton skeleton-card-status" /><span className="loading-skeleton skeleton-card-title" /><span className="loading-skeleton skeleton-card-copy" /><span className="loading-skeleton skeleton-card-meta" /><span className="loading-skeleton skeleton-card-divider" /><span className="loading-skeleton skeleton-card-footer" /></article>)}</section>;
}
